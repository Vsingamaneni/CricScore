...
var db = require('./db');
var userList = require('./util/users');
...
exports.usersList = app.get('/users', async (req, res) => {
    if (req.cookies.loginDetails) {
...
exports.activateMember = app.get('/activate/:memberId', async (req, res) => {
    if (req.cookies.loginDetails) {
        let loginDetails = JSON.parse(req.cookies.loginDetails);
        const memberId = req.params.memberId;


        if (await userList.activateUser(connection, memberId)){
            const msg = [];
            msg.push('User activated successfully!!');
